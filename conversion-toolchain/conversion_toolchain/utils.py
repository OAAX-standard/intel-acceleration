...
from .quantization import quantize_model, is_nncf_available


def extract_input_bundle(input_zip: str, extract_dir: str, logs=None) -> Tuple[str, Optional[str], Optional[str]]:
    """
    ...
    if logs:
        logs.add_message('Validating input archive', {'Path': str(input_path), 'Size': f"{input_path.stat().st_size} bytes"})
    ...
    if not onnx_files:
        ...
        if logs:
            logs.add_message('Error: Model validation failed', {
                'Error': error_msg,
                'Files found': [f.name for f in extract_path.iterdir()]
            })
        raise ValueError(error_msg)

    if len(onnx_files) > 1:
        if logs:
            logs.add_message('Warning: Multiple ONNX files found, using first one', {
                'Files': [str(f) for f in onnx_files],
                'Selected': str(onnx_files[0])
            })
    ...
    if logs:
        logs.add_message('Found ONNX model', {
            'Path': onnx_path,
            'Size': f"{Path(onnx_path).stat().st_size} bytes"
        })
    ...
    for possible_dir in ["calibration", "images", "data"]:
        ...
        if cal_path.exists() and cal_path.is_dir():
            ...
            if logs:
                ...
                logs.add_message('Found calibration data', {
                    'Directory': possible_dir,
                    'Images': image_count
                })
            break
...
def convert_to_ir(
    ...
    try:
        logs.add_message('Starting model conversion', {
            'Model': str(onnx_file.name),
            'Size': f"{onnx_file.stat().st_size} bytes"
        })
        ...
        try:
            ...
            logs.add_message('Error: Model conversion failed', {
                'Error': error_msg,
                'Model': onnx_path,
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            logs.add_message('Saving model to IR format', {
                'FP16 compression': compress_fp16,
                'Quantized': config.is_quantization_enabled()
            })
            ...
            try:
                ...
                logs.add_message('Error: Model save failed', {
                    'Error': error_msg,
                    'Output path': str(xml_path),
                    'Exception type': type(e).__name__
                })
            ...
            if not xml_path.exists() or not bin_path.exists():
                ...
                logs.add_message('Error: Output validation failed', {
                    'XML exists': xml_path.exists(),
                    'BIN exists': bin_path.exists()
                })
                raise IOError(error_msg)

            logs.add_message('Model saved successfully', {
            ...
            try:
                ...
                logs.add_message('Error: Zip creation failed', {
                    'Error': error_msg,
                    'Output path': str(zip_path)
                })
                raise IOError(error_msg) from e
            ...
            logs.add_message('Successfully packaged IR files', {
        ...
        logs.add_message('Failed to convert ONNX model to IR', {
            'Error': str(e)
        })
        raise
