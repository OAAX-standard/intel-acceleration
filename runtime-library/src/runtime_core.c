...
// Create session options
#if OPENVINO_VERSION == 2023
        runtime_core_process_status( api->SetSessionGraphOptimizationLevel( session_options[i], ORT_DISABLE_ALL ) );
        runtime_core_process_status( api->DisableProfiling( session_options[i] ) );
        runtime_core_process_status( api->SetInterOpNumThreads( session_options[i], n_threads_per_duplicate ) );
        ...
        runtime_core_process_status( OrtSessionOptionsAppendExecutionProvider_OpenVINO( session_options[i], "CPU_FP32" ) );
#elif OPENVINO_VERSION == 20241 || OPENVINO_VERSION == 20244
        // Create session options
        int n_options = 2;
        const char *provider_options_keys[] = { "device_type", "precision" };
        const char *provider_options_values[] = { "CPU", "ACCURACY" };
        runtime_core_process_status( api->SessionOptionsAppendExecutionProvider(
                session_options[i], "OpenVINO", provider_options_keys, provider_options_values, n_options ) );
#endif
        char **providers = NULL;
