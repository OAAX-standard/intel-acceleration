...
#define INIT_LOGGER                                                            \
  if (logger == NULL) {                                                        \
    logger =                                                                   \
        create_logger(runtime_name(), "runtime.log", log_level, LOG_INFO);     \
    if (logger == NULL) {                                                      \
...
static int n_threads_per_duplicate = 0;
...
static int runtime_inference_execution(int session_id,
...
int runtime_initialization() {
  ...
  run_options =
      (OrtRunOptions **)malloc(sizeof(OrtRunOptions *) * n_duplicates);
  allocators = (OrtAllocator **)malloc(sizeof(OrtAllocator *) * n_duplicates);
  memory_infos =
      (OrtMemoryInfo **)malloc(sizeof(OrtMemoryInfo *) * n_duplicates);
  envs = (OrtEnv **)malloc(sizeof(OrtEnv *) * n_duplicates);
  session_options =
      (OrtSessionOptions **)malloc(sizeof(OrtSessionOptions *) * n_duplicates);
  sessions = (OrtSession **)malloc(sizeof(OrtSession *) * n_duplicates);
  threads = (pthread_t *)malloc(sizeof(pthread_t) * n_duplicates);

  if (!session_ids || !run_options || !allocators || !memory_infos || !envs ||
      !session_options || !sessions || !threads) {
  ...
  for (int i = 0; i < n_duplicates; i++) {
    ...
    if (runtime_core_process_status(api->CreateEnv(
            ORT_LOGGING_LEVEL_FATAL, "ORT_LOGGER", &envs[i])) != 0) {
      log_error(logger, "Failed to create ORT environment");
      return 1;
    }

    // Create session options
    if (runtime_core_process_status(
            api->CreateSessionOptions(&session_options[i])) != 0) {
    ...
    char *precision = getenv("PRECISION");
    if (precision == NULL) {
      precision = strdup("FP32");
      log_info(logger,
               "PRECISION environment variable not set. Defaulting to %s.\n",
               precision);
    }
    // Read number of threads
    char *num_of_threads = getenv("NUM_OF_THREADS");
    if (num_of_threads == NULL) {
      num_of_threads = strdup("8");
      log_info(
          logger,
          "NUM_OF_THREADS environment variable not set. Defaulting to %s.\n",
          num_of_threads);
    }
    // Read device type
    char *device_type = getenv("DEVICE_TYPE");
    if (device_type == NULL) {
      device_type = strdup("CPU");
      log_info(logger,
               "DEVICE_TYPE environment variable not set. Defaulting to %s.\n",
               device_type);
    }
    int n_options = 5;
    const char *provider_options_keys[] = {"device_type", "precision",
                                           "num_of_threads", "num_streams",
                                           "enable_opencl_throttling"};
    const char *provider_options_values[] = {device_type, precision,
                                             num_of_threads, "1", "false"};
    runtime_core_process_status(api->SessionOptionsAppendExecutionProvider(
        session_options[i], "OpenVINO", provider_options_keys,
        provider_options_values, n_options));
    char **providers = NULL;
    int number_providers = 0;
    runtime_core_process_status(
        api->GetAvailableProviders(&providers, &number_providers));
    for (int j = 0; j < number_providers; ++j)
      log_info(logger, "Provider id: %i - name: %s", j, providers[j]);
    runtime_core_process_status(
        api->ReleaseAvailableProviders(providers, number_providers));

    // create run options
    if (runtime_core_process_status(api->CreateRunOptions(&run_options[i])) !=
        0) {
  ...
  log_debug(logger,
            "n_duplicates: %d - n_threads_per_duplicate: %d - log_level: %d",
...
int runtime_model_loading(const char *file_path) {
  ...
  for (int i = 0; i < n_duplicates; i++) {
    session_ids[i] = i;
    if (runtime_core_process_status(api->CreateSession(
            envs[i], file_path, session_options[i], &sessions[i])) != 0) {
      log_error(logger, "Failed to create session");
      return 1;
    }

    // create allocator
    if (runtime_core_process_status(api->CreateCpuMemoryInfo(
            OrtArenaAllocator, OrtMemTypeDefault, &memory_infos[i])) != 0) {
      log_error(logger, "Failed to create memory info");
      return 1;
    }

    if (runtime_core_process_status(api->CreateAllocator(
            sessions[i], memory_infos[i], &allocators[i])) != 0) {
  ...
  for (int i = 0; i < n_duplicates; i++) {
    if (pthread_create(&threads[i], NULL, inference_loop, &session_ids[i]) !=
        0) {
...
static void *inference_loop(void *arg) {
  ...
  while (1) {
    tensors_struct *input_tensors =
        dequeue(input_queue,
                200); // timeout after 200ms to check if the thread should stop
    if (input_tensors != NULL) {
      ...
      exit_code = runtime_inference_execution(session_id, input_tensors,
                                              output_tensors);
      if (exit_code != 0) {
        log_warning(logger, "Inference execution failed with code %d",
                    exit_code);
...
static int runtime_inference_execution(int session_id,
  ...
  input_names =
      runtime_core_get_input_names(sessions[session_id], allocators[session_id],
                                   &number_inputs, &input_dtypes);
  ...
  // Make sure that the number of input tensors is the same as the one passed to
  // the runtime
  if (number_inputs != (int)input_tensors->num_tensors) {
    log_error(logger, "The number of input tensors does not match the number "
                      "of input tensors in the model");
    log_error(logger, "Expected: %d, Got: %zu", number_inputs,
  ...
  output_names = runtime_core_get_output_names(sessions[session_id],
                                               allocators[session_id],
                                               &number_outputs, &output_dtypes);
  ...
  for (int i = 0; i < number_inputs; ++i) {
    RUNTIME_ORT_CORE_EXEC(
        runtime_core_process_status(api->CreateTensorWithDataAsOrtValue(
            memory_infos[session_id], inputs[i],
            input_sizes[i] * runtime_util_get_sizeof_onnx_type(input_dtypes[i]),
            input_shapes[i], input_ranks[i], input_dtypes[i],
            &input_values[i])),
        error);
  }
  ...
  RUNTIME_ORT_CORE_EXEC(
      runtime_core_process_status(api->Run(
          sessions[session_id], run_options[session_id],
  ...
  for (int i = 0; i < number_outputs; ++i) {
    ...
    RUNTIME_ORT_CORE_EXEC(
        runtime_core_process_status(
            api->GetTensorTypeAndShape(output_values[i], &type_shape)),
        error);
    ...
    RUNTIME_ORT_CORE_EXEC(
        runtime_core_process_status(
            api->GetTensorShapeElementCount(type_shape, &size)),
        error);

    // get output rank
    size_t rank = 0;
    RUNTIME_ORT_CORE_EXEC(
        runtime_core_process_status(api->GetDimensionsCount(type_shape, &rank)),
        error);
    output_ranks[i] = rank;
    ...
    RUNTIME_ORT_CORE_EXEC(runtime_core_process_status(api->GetDimensions(
                              type_shape, (int64_t *)output_shapes[i], rank)),
                          error);

    // get output value
    size_t output_bytes =
        size * runtime_util_get_sizeof_onnx_type(output_dtypes[i]);
    ...
    void *tmp = NULL;
    RUNTIME_ORT_CORE_EXEC(runtime_core_process_status(api->GetTensorMutableData(
                              output_values[i], &tmp)),
                          error);
...
const char *runtime_version() { return "0.1.0"; }

const char *runtime_name() { return "OnnxRuntime"; }
